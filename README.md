...
The _doc_ SDK can be hosted on any domain simply by serving the appropriate index.html created during the build process. Before serving the file, the _{{{content}}}_ mustache tag has to be replaced with either _null_ or a _document object_ describing which dataset to load intially. Much of the document object is created by the Voxxlr processing pipeline and typically stored server side. The object must also include a source attribute wich contains a _data url_ to the output generated by the processing pipeline as well as _refresh url_ should the data _url expire_. When running _server.js_, the document object retrieved from the hosted version at Voxxlr via [doc.voxxlr.com](https://doc.voxxlr.com/rest.html) REST endpoint is output to the console. 
