...
V2.Viewer = class extends V.Viewer
{
    constructor()
    {
        ...
        { 
            ...
            {
                let map;
                if (document.type == V.Dataset.MOSAIC)
                {
                    map = new M.Map(document, V.importCb.bind(config), pane)
                }
                else if (document.type == V.Dataset.WMTS)
                {
                    map = new M.Wmts(document, V.importCb.bind(config), pane)
                }
                else if (document.type == V.Dataset.IMAGE)
                {
